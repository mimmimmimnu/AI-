        ...
        child: ConstrainedBox(
          ...
          child: SingleChildScrollView(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.center, children: [
              ...
              _PrimaryButton(label: '로그인', onTap: () {
              ...
              const SizedBox(height: 20),
              Row(children: [
                const Expanded(child: Divider(color: kNude, thickness: 1.5)),
                Padding(padding: const EdgeInsets.symmetric(horizontal: 12),
                  child: Text('또는', style: GoogleFonts.dmSans(fontSize: 12, color: kMuted))),
                const Expanded(child: Divider(color: kNude, thickness: 1.5)),
              ]),
              const SizedBox(height: 20),
      ...
      body: SafeArea(child: Column(children: [
        ...
        Expanded(child: SingleChildScrollView(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            const SizedBox(height: 10),
            _CourseCard(emoji: '🌸', color1: kRoseLt, color2: const Color(0xFFC5D5F7),
              title: '봄날 홍대 산책 코스', sub: '카페 → 전시 → 한강  ·  3곳  ·  4시간'),
            const SizedBox(height: 10),
            _CourseCard(emoji: '🍃', color1: const Color(0xFFC5F7D5), color2: const Color(0xFFF7F0C5),
              title: '연남동 브런치 데이트', sub: '식사 → 카페 → 쇼핑  ·  3곳  ·  3시간'),
            const SizedBox(height: 20),
          ]),
        )),
      ])),
...
class _CoursePageState extends State<CoursePage> with SingleTickerProviderStateMixin {
  ...
  static const String _groqApiKey        = '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';
...
Output ONLY a JSON array with EXACTLY 6 objects. No markdown, no explanation, no extra text.

$themeLine
Region: $region$hotspotHint$mandatoryNote$spotsContext$restaurantCtx$userPreferences

MANDATORY: Output exactly these 6 bands in this order. Missing any band = invalid response.
1. band="morning"    cafe/park/exhibition (10:00 start)
2. band="lunch"      meal restaurant ONLY (12:00 fixed) category="FD6" engine="kakao"
3. band="afternoon"  shopping/dessert/experience
4. band="afternoon2" exhibition/walk/landmark (different type from afternoon)
5. band="dinner"     meal restaurant ONLY (18:00 fixed) category="FD6" engine="kakao"
6. band="night"      night view/rooftop/riverside (20:00 start)

Rules:
- search field: "<neighborhood> <specific feature>" (e.g. "이태원 루프탑 바")
- No duplicate place names
- duration: activities="1시간 30분", meals="1시간"

Output exactly this structure:
[{"band":"morning","emoji":"☕","type":"카페","duration":"1시간 30분","description":"이태원 감성 카페","search":"이태원 감성 브런치 카페","category":"CE7","engine":"naver"},
{"band":"lunch","emoji":"🍽️","type":"점심식사","duration":"1시간","description":"이태원 맛집","search":"이태원 점심 맛집","category":"FD6","engine":"kakao"},
...
''';
  ...
  Future<int?> _getTravelMinutes(PlaceResult from, PlaceResult to) async {
    ...
    return math.max(1, (distKm * 1.5 / 4.0 * 60).ceil());
  }
  ...
  Future<void> _recalculateTimes() async {
    ...
    for (int i=1; i<_courseSteps.length; i++) {
      ...
      final band=curr.band; final type=curr.type;
      if (band=='lunch'||type.contains('점심')) cursor=math.max(cursor,12*60);
      if (band=='dinner'||type.contains('저녁')) cursor=math.max(cursor,18*60);
      if (band=='night'||type.contains('야간')||type.contains('야경')) cursor=math.max(cursor,20*60);
                  ...
                  onPressed: isSaving ? null : () async {
                    ...
                    await FirebaseFirestore.instance.collection('place_comments').add({
                      ...
                      'username': FirebaseAuth.instance.currentUser?.displayName ??
                          FirebaseAuth.instance.currentUser?.email ?? '익명',
    ...
    return Scaffold(
      ...
      body: SafeArea(child: Column(children: [
        ...
        Expanded(child: SingleChildScrollView(padding: const EdgeInsets.symmetric(horizontal: 20), child: Column(children: [
          ...
          const SizedBox(height: 10),
          ...user.savedCourses.take(2).toList().asMap().entries.map((e) => _SavedCourseCard(course: e.value, onDelete: () => user.deleteCourse(e.key))),
          const SizedBox(height: 20),
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            _SectionLabel('친구'),
