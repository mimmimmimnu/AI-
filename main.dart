...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> _login() async {
    ...
    setState(() { _nicknameError = ''; _passwordError = ''; });
    ...
    try {
      ...
      if (query.docs.isNotEmpty) {
        // 기존 유저 — 비번 확인
        final savedPassword = query.docs.first.data()['password'] as String? ?? '';
        if (savedPassword != password) {
          setState(() { _passwordError = '비밀번호가 틀렸어요 🔒'; _isLoading = false; });
          return;
        }
        // 비번 일치 → 로그인
        if (mounted) {
          context.read<UserProvider>().updateName(nickname);
      ...
      setState(() { _nicknameError = '오류: $e'; _isLoading = false; });
    } finally {
        ...
        child: ConstrainedBox(
          ...
          child: SingleChildScrollView(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.center, children: [
              ...
              Text('✿ Datefit',
                style: GoogleFonts.playfairDisplay(fontSize: 36, color: kRoseDk, fontWeight: FontWeight.w600)),
              const SizedBox(height: 8),
              Text('둘이서 만드는 완벽한 하루',
                style: GoogleFonts.dmSans(fontSize: 13, color: kMuted, letterSpacing: 0.5)),
              ...
              TextField(
                ...
                decoration: InputDecoration(
                  ...
                  errorStyle: GoogleFonts.dmSans(fontSize: 12, color: kRose),
                ),
              ),
...
class _ReviewTabState extends State<ReviewTab> {
  ...
  void initState() {
    super.initState();
    // 앱 시작 시 기존 후기 마커 로드
    WidgetsBinding.instance.addPostFrameCallback((_) => _loadCommentMarkers());
  }
  ...
  void _showPlaceOnMap(dynamic store) {
    ...
    globalContext.callMethod('moveMapTo'.toJS, lat.toJS, lng.toJS, name.toString().toJS);
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Row(children: [
        ...
        const VerticalDivider(width: 1),
        // ── 오른쪽 지도 ──
        const Expanded(child: HtmlElementView(viewType: 'kakao-map-view')),
      ]),
    );
  }
}
