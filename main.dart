// ============================================================
// Course It — Firebase 인증/Firestore 연동 + 모바일(Android/iOS) 전환본
// 💡 변경: 카카오 웹 지도(JS interop, HtmlElementView) → google_maps_flutter 네이티브 위젯으로 교체
//         웹 파일선택(HTMLInputElement) → image_picker 로 교체
//         web.window.open → url_launcher 로 교체
//         (Firebase, Firestore, Groq AI, 카카오 Local API 장소검색, 코스 생성 로직은 변경 없음)
// ============================================================
import 'dart:async';
import 'dart:convert';
...
// ────────────────────────────────────────────────────────────
// 앱 진입
// 💡 변경: 웹 전용 platformViewRegistry.registerViewFactory(카카오맵 JS 브릿지) 블록 제거.
//         네이티브 지도(google_maps_flutter)는 이런 초기화가 필요 없음.
// ────────────────────────────────────────────────────────────
void main() async {
...
// ────────────────────────────────────────────────────────────
// 메인 쉘 (탭 구조)
// 💡 변경: 후기 탭(3번) 전환 시 호출하던 카카오 웹 지도 리레이아웃 JS 호출 제거
//         (네이티브 지도는 그런 리레이아웃 트릭이 필요 없음)
// ────────────────────────────────────────────────────────────
...
// ────────────────────────────────────────────────────────────
// AI 코스 추천 페이지
// 💡 변경: HtmlElementView(카카오 웹 지도) → FlutterMap(OpenStreetMap) 위젯
//         globalContext.callMethod(clearMarkers/addCourseMarker/drawRoute/moveMapTo)
//         → List<Marker>/List<Polyline> 로컬 상태 + MapController (API 키·카드 불필요)
//         web.window.open → url_launcher
// ────────────────────────────────────────────────────────────
...
class _CoursePageState extends State<CoursePage> with SingleTickerProviderStateMixin {
  ...
  // 💡 네이티브 지도 상태 (flutter_map / OpenStreetMap — API 키·카드 필요 없음)
  final MapController _mapController = MapController();
  List<Marker> _markers = [];
  List<Polyline> _polylines = [];

  // 💡 탭 콘텐츠 / 지도 영역 사이 경계를 드래그로 리사이즈할 수 있게 하는 상태값
  //    (갤럭시탭 분할화면처럼 사용자가 자유롭게 두 영역의 크기를 조절)
  static const double _kMapHeightMin = 120;
  double _mapHeight = 260;

  Marker _buildCourseMarker(PlaceResult place, int number) {
    return Marker(
      ...
      child: GestureDetector(
        ...
        child: Container(
          decoration: BoxDecoration(color: kRose, shape: BoxShape.circle, border: Border.all(color: Colors.white, width: 2)),
...
''';
  ...
  String _buildCoursePrompt({
    ...
    final shownRegion = displayRegion ?? region;
    return '''
...
''';
  ...
  void initState() {
    ...
    // 💡 탭 복귀 시 저장해둔 코스 스텝/경로로 마커·폴리라인 즉시 복원 (웹의 JS SDK 로딩 대기 루프는 불필요해짐)
    if (_savedCourseSteps.isNotEmpty) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (!mounted) return;
        _redrawAllMarkers();
        if (_savedRouteSegments != null) _applyRouteSegments(_savedRouteSegments!);
      });
    }
  }
  ...
  // 💡 마커 헬퍼: 카카오 웹 JS 호출을 대체하는 로컬 상태 조작 함수들
  ...
  void _addCourseMarker(PlaceResult place, int number) {
    ...
    setState(() => _markers.add(_buildCourseMarker(place, number)));
  }
  ...
  void _redrawAllMarkers() {
    ...
    for (int i = 0; i < _courseSteps.length; i++) {
      ...
      if (p?.lat != null && p?.lng != null) newMarkers.add(_buildCourseMarker(p!, i + 1));
    }
    setState(() => _markers = newMarkers);
    _fitMapToMarkers();
  }

  void _applyRouteSegments(List<List<Map<String, double>>> segments) {
    final newPolylines = <Polyline>[];
    for (final seg in segments) {
      final pts = seg.map((pt) => LatLng(pt['lat']!, pt['lng']!)).toList();
      if (pts.length < 2) continue;
      newPolylines.add(Polyline(points: pts, color: kRose, strokeWidth: 4));
    }
    setState(() => _polylines = newPolylines);
  }

  // 마커들이 다 보이도록 카메라를 자동으로 맞춰줌
  void _fitMapToMarkers() {
    ...
    double minLat = _markers.first.point.latitude, maxLat = minLat;
    double minLng = _markers.first.point.longitude, maxLng = minLng;
    for (final m in _markers) {
      minLat = math.min(minLat, m.point.latitude);
      maxLat = math.max(maxLat, m.point.latitude);
      minLng = math.min(minLng, m.point.longitude);
      maxLng = math.max(maxLng, m.point.longitude);
    }
    try {
  ...
  Future<void> searchPlaces(String query, {String categoryCode = '', String engine = 'naver', bool fromChat = false}) async {
    ...
    try {
      ...
      setState(() {
        final valid = results.where((p) => p.lat != null && p.lng != null).toList();
        _markers = List.generate(valid.length, (i) => _buildCourseMarker(valid[i], i + 1));
      });
  ...
  Future<void> _generateCourse({String? queryOverride}) async {
    ...
    try {
      ...
      setState(() {
        ...
        if (_courseSteps.isEmpty) {
          _courseError = '"$regionLabel" 근처에서 장소를 찾지 못했어요.\n카카오/네이버 API 키를 확인하거나 다른 동네로 다시 시도해보세요.';
        }
      });
  ...
  Future<List<PlaceResult>> _searchStepCandidates(CourseStep step, String region, double? cLat, double? cLng) async {
    ...
    bool inDistrict(PlaceResult p) => region.isEmpty ? true : '${p.address} ${p.roadAddress}'.contains(region);
    ...
    if (best.isEmpty && cLat != null && cLng != null) {
      for (final rad in [2000, 5000]) {
        try {
          final r = _filter(await _searchKakao(step.searchQuery, step.categoryCode, centerLat: cLat, centerLng: cLng, radius: rad));
          if (r.isNotEmpty) { best = r; break; }
        } catch (_) {}
      }
    }
    return best;
  ...
  Future<void> _replaceFarSteps(String region) async {
    for (int i = 1; i < _courseSteps.length; i++) {
      ...
      if (curr.searchQuery.isEmpty) continue;
            ...
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              Padding(padding: const EdgeInsets.fromLTRB(20, 0, 20, 24), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Row(children: [
                  ...
                  Expanded(child: OutlinedButton.icon(
                    onPressed: () async {
                      final uri = Uri.parse('https://map.naver.com/v5/search/${Uri.encodeComponent(place.name)}');
                      if (await canLaunchUrl(uri)) await launchUrl(uri, mode: LaunchMode.externalApplication);
                    },
  ...
  @override
  Widget build(BuildContext context) {
    // 💡 키보드가 열려있는 동안엔 하단 지도를 숨겨서, 화면이 밀리거나
    //    레이아웃이 넘치는(overflow) 현상 없이 채팅 입력창이 안정적으로 보이게 함
    final keyboardOpen = MediaQuery.of(context).viewInsets.bottom > 0;
    final screenHeight = MediaQuery.of(context).size.height;
    final mapHeightMax = screenHeight * 0.7;
    final mapHeight = _mapHeight.clamp(_kMapHeightMin, mapHeightMax);
    return Scaffold(
      backgroundColor: kCream,
      appBar: AppBar(
        backgroundColor: Colors.white,
      ...
      body: Column(children: [
        ...
        if (!keyboardOpen) ...[
          // 💡 지도는 모바일 화면 하단에 배치 (웹처럼 좌우 분할 대신 상하 분할).
          //    아래 핸들을 드래그하면 갤럭시탭 분할화면처럼 탭 콘텐츠/지도 영역의
          //    크기를 자유롭게 조절할 수 있음 (앱 재시작 시 기본값 260px로 초기화됨)
          MouseRegion(
            ...
            child: GestureDetector(
              ...
              onVerticalDragUpdate: (details) {
                setState(() {
                  _mapHeight = (_mapHeight - details.delta.dy).clamp(_kMapHeightMin, mapHeightMax).toDouble();
                });
              },
              child: Container(
                height: 20,
          ...
          SizedBox(height: mapHeight.toDouble(), child: FlutterMap(
            ...
            children: [
              TileLayer(
                ...
                userAgentPackageName: 'com.example.datefit',
              ),
  ...
  Widget _buildChatTab() {
    return Column(children: [
      ...
      Expanded(
        child: ListView.builder(controller: chatScrollController, padding: const EdgeInsets.all(12),
...
// ────────────────────────────────────────────────────────────
// 타임라인 탭
// 💡 변경: 사진 첨부 로직을 웹 파일선택(HTMLInputElement) → image_picker 로 교체
// ────────────────────────────────────────────────────────────
...
class _TimelineTabState extends State<TimelineTab> {
        ...
        child: _BottomSheet(children: [
          ...
          if (pickedBytes != null) ...[
            ClipRRect(borderRadius: BorderRadius.circular(14),
              child: Stack(children: [
                Image.memory(pickedBytes!, width: double.infinity, height: 200, fit: BoxFit.cover),
  ...
  // 💡 변경: 갤러리에서 사진 선택 (image_picker). maxWidth/imageQuality로 리사이즈·압축까지 자동 처리됨
...
// 후기 탭
// 💡 변경: HtmlElementView(카카오 웹 지도) → FlutterMap(OpenStreetMap) 위젯
//         globalContext.callMethod(clearCommentMarkers/addCommentMarker/moveReviewMapTo)
//         → List<Marker> 로컬 상태 + MapController (API 키·카드 불필요)
//         (HTML 팝업 대신 마커 탭 시 _showReviewedPlaceDialog로 후기 목록 다이얼로그 표시)
// ────────────────────────────────────────────────────────────
...
class _ReviewTabState extends State<ReviewTab> {
  ...
  // 💡 네이티브 지도 상태 (flutter_map / OpenStreetMap)
  final MapController _reviewMapController = MapController();
  List<Marker> _reviewMarkers = [];
  ...
  void _redrawCommentMarkers() {
    ...
    for (final p in _reviewedPlaces) {
      ...
      newMarkers.add(Marker(
        ...
        width: 38, height: 38,
        child: GestureDetector(
          onTap: () => _showReviewedPlaceDialog(context, p),
          child: Container(
            decoration: BoxDecoration(color: Colors.white, shape: BoxShape.circle, border: Border.all(color: kRose, width: 2)),
            child: const Center(child: Text('⭐', style: TextStyle(fontSize: 16))),
          ),
        ),
      ));
    }
    if (mounted) setState(() => _reviewMarkers = newMarkers);
  }
  ...
  void _showPlaceOnMap(dynamic store) {
    ...
    _reviewMapController.move(LatLng(lat, lng), 16);
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SafeArea(child: Column(children: [
        ...
        // 💡 모바일에서는 지도를 위쪽, 검색/목록을 아래쪽으로 상하 배치
        SizedBox(height: 220, child: FlutterMap(
          ...
          children: [
            ...
            MarkerLayer(markers: _reviewMarkers),
          ],
        )),
        const Divider(height: 1),
        Expanded(child: SingleChildScrollView(child: Column(children: [
          ...
          const SizedBox(height: 8),
          if (_isLoading)
            const Padding(padding: EdgeInsets.all(24), child: CircularProgressIndicator(color: kRose))
